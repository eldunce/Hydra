...
namespace Hydra
{
    public class PoolManager : MonoBehaviour {
        ...
        void TrackSpawnedObject(Transform t, Transform prefab, Pool.UpdateDelegate update, Pool.DespawnTestDelegate despawn)
        {
            if (mSpawnedObjects.ContainsKey(t))
            {
                UntrackSpawnedObject(t);
            }            
            else
            {
                mSpawnedObjects[t] = mRuntimePools[prefab];
            }
            if (update != null)
            {
            ...
            if (despawn != null)
            {
                ...
                check.del = despawn;
                check.t = t;
                mDespawnChecks.Add(check);
            }
        }

        void UntrackSpawnedObject(Transform t)
        {
            int idx = mUpdates.FindIndex((x) => x.t == t);
            if (idx != -1)
                mUpdates.RemoveAt(idx);
